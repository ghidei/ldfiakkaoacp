...
class RaftClusterListener(actor: ActorRef) extends Actor with ActorLogging{
  ...
  override def preStart(): Unit = {
    ...
    if (Controller.greenLight(self.path.name, actor.path.name, ClusterListenerIs(self))) actor ! ClusterListenerIs(self)
  ...
  def receive() = LoggingReceive {
    ...
    case MemberRemoved(member, _) if member.hasRole("raft") =>
      ...
      if (Controller.greenLight(self.path.name, actor.path.name, RaftMemberDeleted(member.address))) actor ! RaftMemberDeleted(member.address)

    case ActorIdentity(_, ref) if ref.isDefined => //ActorRef as member
      log.warning("actorIdentity excuted")
      if (Controller.greenLight(self.path.name, actor.path.name, RaftMemberAdded(ref.get))) actor ! RaftMemberAdded(ref.get) //FIXME: how to delete?
    ...
    case msg =>
      if (Controller.greenLight(self.path.name, actor.path.name, msg)) actor.tell(msg, sender())
  }
